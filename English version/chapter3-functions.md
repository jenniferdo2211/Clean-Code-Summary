...
Not only does this keep the enclosing function small but it also adds documentary value because the function called within the bloack can have a nicely descriptive name.
* Functions should not be large enough to hold a nested structure -> max indent level is 1 or 2 and all lines are at the same abstraction level.
...
Eg. Listing 3-3: HtmlUtil.java is described as...<br/>
...
Explanation:<br/>>
* Before, we have 3 functions `isPayday, calculatePay, deliverPay`, in each function, we have `switch... case` statement to call different functions based on different types of employees.<br /> We divide our cases based on functions and then by employee types.
